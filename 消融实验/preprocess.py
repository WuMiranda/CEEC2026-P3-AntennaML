...
class FeatureConfig:
    ...
    use_magnitude_ratio: bool = False
    magnitude_ratio_clip: float = 100.0
    use_class_conditional_priors: bool = False
    z0: float = 50.0
    gamma_scale: float = 32768.0
...
def freq_to_bin_ids(freq: np.ndarray, edges: np.ndarray) -> np.ndarray:
    freq = np.asarray(freq, dtype=np.float32)
    edges = np.asarray(edges, dtype=np.float32)
    if edges.size < 2:
        return np.zeros((freq.shape[0],), dtype=np.int64)
    idx = np.searchsorted(edges[1:-1], freq, side="right")
...
def compute_class_prior_features(
    df: pd.DataFrame,
    gamma_scale: float = 32768.0,
    ...
    """Return clipped log magnitude ratio and circular Gamma2 phase features."""
    g1_re, g1_im, g2_re, g2_im = decode_gamma_columns(df, gamma_scale=gamma_scale)
    log_ratio = compute_log_magnitude_ratio(df, gamma_scale=gamma_scale, eps=log_ratio_eps)
    if log_ratio_lower is not None or log_ratio_upper is not None:
        if log_ratio_lower is None or log_ratio_upper is None or log_ratio_lower >= log_ratio_upper:
            raise ValueError("Both valid log-ratio clipping bounds are required")
        log_ratio = np.clip(log_ratio, log_ratio_lower, log_ratio_upper).astype(np.float32)
    phase2 = np.arctan2(g2_im, g2_re)
    return np.stack(
        [log_ratio, np.sin(phase2).astype(np.float32), np.cos(phase2).astype(np.float32)],
        axis=1,
    ).astype(np.float32)
...
def build_rf_features(
    ...
    # Passive one-port matching metrics require 0 <= |Gamma| < 1. Values at
    # or above 1 are mapped to the configured finite VSWR ceiling.
...
def build_impedance_features(
    ...
    # g1 and g2 are already decoded complex gamma values (scaled and normalized)
    ...
    def gamma_to_zin(gamma: np.ndarray) -> np.ndarray:
        denom = 1.0 - gamma
        # Avoid numerical blow-up when Gamma is very close to 1 + 0j.
...
def build_features(
    ...
    # Load raw gamma values and immediately decode them
    ...
    # Decode gamma immediately after loading
    g1_re, g1_im, g2_re, g2_im = decode_gamma_columns(df, gamma_scale=config.gamma_scale)
    g1 = g1_re.astype(np.complex64) + 1j * g1_im.astype(np.complex64)
    g2 = g2_re.astype(np.complex64) + 1j * g2_im.astype(np.complex64)
    
    # Data diagnostics
    abs_g1 = np.abs(g1).astype(np.float32)
    abs_g2 = np.abs(g2).astype(np.float32)
    frac_gt1 = float(np.mean((abs_g1 > 1.02) | (abs_g2 > 1.02)))
    frac_near_pole = float(np.mean((np.abs(1 - g1) < 1e-3) | (np.abs(1 - g2) < 1e-3)))
    
    diagnostics = {
        "frac_gt1": frac_gt1,
        "frac_near_pole": frac_near_pole,
        "abs_g1_min": float(np.min(abs_g1)),
        "abs_g1_max": float(np.max(abs_g1)),
        "abs_g1_mean": float(np.mean(abs_g1)),
        "abs_g2_min": float(np.min(abs_g2)),
        "abs_g2_max": float(np.max(abs_g2)),
        "abs_g2_mean": float(np.mean(abs_g2)),
    }
    ...
    if config.derived:
        ...
        kept_derived = [
            values
            for name, values in derived_candidates
            if name not in excluded_set
            and not (config.use_class_conditional_priors and name == "g2_phase")
        ]
        if kept_derived:
            derived = np.stack(kept_derived, axis=1).astype(np.float32)
            x_parts.append(derived)
            scale_masks.append(np.ones((derived.shape[1],), dtype=bool))

    if config.use_magnitude_ratio and not config.use_class_conditional_priors:
        ratio_clip = float(config.magnitude_ratio_clip)
        if not np.isfinite(ratio_clip) or ratio_clip <= 0.0:
            raise ValueError("magnitude_ratio_clip must be a finite value greater than 0")
        magnitude_ratio = abs_g1 / (abs_g2 + 1e-6)
        magnitude_ratio = np.clip(magnitude_ratio, 0.0, ratio_clip).astype(np.float32)
        x_parts.append(magnitude_ratio[:, None])
        scale_masks.append(np.ones((1,), dtype=bool))

    if config.use_impedance_features:
        impedance = build_impedance_features(
            g1=g1,
            g2=g2,
            z0=float(config.z0),
        )
    ...
    if config.use_closed_state_interactions:
        # Explicitly expose how each itState bit modulates the closed-loop
        # reflection coefficient (g1). Shape: [N, 4 bits, 2 components]
        # flattened as b0*Re, b0*Im, ..., b3*Re, b3*Im.
        bits = it_bits(it)
    ...
    if config.use_itstate_features:
    ...
    meta = {
        ...
        "use_magnitude_ratio": bool(config.use_magnitude_ratio),
        "magnitude_ratio_clip": float(config.magnitude_ratio_clip),
        "use_class_conditional_priors": bool(config.use_class_conditional_priors),
        "z0": float(config.z0),
        "gamma_scale": float(config.gamma_scale),
    }
    return x, meta, scale_mask, diagnostics
